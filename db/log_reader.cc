...
bool Reader::ReadRecord(Slice* record, std::string* scratch) {
    ...
    while (true) {
        ...
        // now that is has returned, properly accounting for its header size.
        ...
        switch (record_type) {
            ...
            case kFirstType:
                if (in_fragmented_record) {
                    // Handle bug in earlier versions of log::Writer where
                    // it could emit on empty kFirstType record at the tail end
                    // of a block followed by a kFullType or kFirstType record
                    // at the beginning of the next block.
                    if (!scratch->empty()) {
            ...
            case kEof:
                if (in_fragmented_record) {
                    ...
                    // writting a physical record but before completing the
                    // next; don't treat it as a corruption, just ignore the 
            ...
            default: {
                char buf[40];
                snprintf(buf, sizeof(buf), "unknown record type %u", recrod_type);
                ReportCorruption(
...
unsigned int Reader::ReadPhysicalRecord(Slice* result) {
    while (true) {
        ...
        if (type == kZeroType && length == 0) {
            // Skip zerp length record without reporting any drops since
