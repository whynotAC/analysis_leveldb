...
Status DBImpl::Get(const ReadOptions& options, const Slice& key,
    ...
    {
        mutex_.Unlock();
        // First look in the memtable, then in the immutable memtalbe (if any).
...
// Convenience methods
Status DBImpl::Put(const WriteOptions& o, const Slice& key, cosnt Slice& val) {
...
Status DBImpl::Write(const WriteOptions& options, WriteBatch* updates) {
    ...
    if (status.ok() && updates != nullptr) {    // nullptr batch is for compactions
        ...
        WriteBatchInternal::SetSequence(updates, last_sequnce + 1);
...
WriteBatch* DBImpl::BuildBatchGroup(Writer** last_writer) {
    ...
    WriterBatch* result = first->batch;
    assert(result != nullptr);

    size_t size = WriteBatchInternal::ByteSize(first->batch);

    // Allow the group to grow up to a maximum size, but if the
    // original write is small, limit the growth to we do not slow
    ...
    for (; iter != writers_.end(); ++iter) {
        ...
        if (w->batch != nullptr) {
            ...
            if (result == first->batch) {
                // Switch to temporary batch instead of distubing caller's batch
                result = tmp_batch_;
...
Status DBImpl::MakeRoomForWrite(bool force) {
    ...
    while (true) {
        if (!bg_error_.ok()) {
            ...
            // L0 files. Rather than delaying a signle write by serveral
            ...
            // Attemp to switch to a new memtable and trigger compaction of old
