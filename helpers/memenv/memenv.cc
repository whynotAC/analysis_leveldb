...
class FileState {
  public:
    ...
    // and the caller must all Ref() at least once.
    ...
    Status Read(uint64_t offset, size_t n, Slice* result, char* scratch) const {
        ...
        assert(offset / kBlockSize <= std::number_limits<size_t>::max());
...
class RandomAccessFileImpl : public RandomAcessFile {
  public:
...
class InMemoryEnv : public EnvWrapper {
  ...
  private:
    // Map from filenames to FileState objects, representing a single file
    // sytem.
